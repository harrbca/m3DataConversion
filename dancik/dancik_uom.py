...
class UOMService:
    ...
    def _build_bidirectional_conversion_graph(self):
        ...
        if(item_details["ICOMPO"]) == 'R':
            ...
            self._add_conversion_to_graph(9, "IN", "SY")
            width = Decimal(str(item_details["IWIDTH"]))
            if width == 0:
                width = Decimal("72")
            width = width / Decimal("12")
            self._add_conversion_to_graph(width, "SF", "LF")
    ...
    def convert(self, value, from_uom, to_uom):
        ...
        rounded_result = dec_value.quantize(Decimal(f"0.{'0' * self.final_result_precision}"), rounding=ROUND_HALF_DOWN)
